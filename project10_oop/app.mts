...
function welcome() {
  ...
  figlet(msg, (err, data) => {
    ...
    rainbowTitle = chalkAnimation.rainbow(
      "Welcome to OOP Example! (A CLI based oop example developed by amlakhani)"
    );
  });
}
...
async function startProgram() {
  ...
  if (isNaN(parseInt(answer))) {
    ...
    student.askQuestion(parseInt(answer));
    console.log(chalk.green(`You are: ${student.getPersonality()}`));
    ...
    console.log(
      chalk.green(
        `You name is: ${
          student._name
        } and you personality is ${student.getPersonality()}.`
      )
    );
  }
}
