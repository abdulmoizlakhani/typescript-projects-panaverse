...
function welcome() {
    ...
    figlet(msg, (err, data) => {
        ...
        rainbowTitle = chalkAnimation.rainbow("Welcome to Dungeon Master! (A CLI based adventure game developed by amlakhani)");
    });
}
async function startGame() {
    ...
    while (player.running) {
        ...
        GAME: while (enemy.enemyHealth > 0) {
            ...
            console.log(`   ${enemy.enemyName}'s HP: ${chalk.blue(enemy.enemyHealth)}\n`);
            ...
            if (option === "Attack") {
                ...
                if (player.numHealthPotions > 0) {
                    player.usePotion();
                    console.log(chalk.green(`   > You drank a health potion, healing yourself for ${player.healthPotionHealAmount}.\n`));
                    console.log(chalk.green(`   > You now have ${player.health} HP.\n`));
                    console.log(chalk.green(`   > You have ${player.numHealthPotions} health potions left.\n`));
                }
                else {
                ...
                console.log(chalk.blue(`   You run away from the enemy ${enemy.enemyName}!\n`));
                continue GAME;
            }
        }
        if (player.health < 1) {
        ...
        console.log(chalk.white.bgGreen(`   # You have ${player.health} HP left. #\n`));
        if (utils.getRandomNumber(100) < player.healthPotionDropChance) {
            player.takeEnemyHealthPotion();
            console.log(chalk.green(`   # The ${enemy.enemyName} dropped a health potion! #\n`));
            console.log(chalk.green(`   # You now have ${player.numHealthPotions} health potion(s). #\n`));
        }
        const { option } = await inquirer.prompt({
            name: "option",
            type: "list",
