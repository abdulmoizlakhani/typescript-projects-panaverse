...
function welcome() {
    ...
    figlet(msg, (err, data) => {
        ...
        rainbowTitle = chalkAnimation.rainbow("Welcome to Currency Converter! (A CLI based currency converter app developed by amlakhani)");
    });
}
...
async function askForOperation() {
    ...
    if (answer.operation === "Exit") {
        ...
        if (currency === "Exit") {
            ...
            const selectedCurrency = currenciesList.find((c) => c.currency === currency2);
            console.log(chalk.white.bgBlue(`1 ${currency2} = ${selectedCurrency["exchangeRates"][currency]} ${currency} \n`));
            console.log("\nYou will get:");
            console.log(chalk.white.bgBlue(`${amount} ${currency2} = ${selectedCurrency["exchangeRates"][currency] * amount} ${currency} \n`));
        }
    }
}
try {
