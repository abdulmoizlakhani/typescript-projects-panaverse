...
import { Student, Course } from "./interfaces/index.js";
...
function welcome() {
  ...
  figlet(msg, (err, data) => {
    ...
    rainbowTitle = chalkAnimation.rainbow(
      "Welcome to Students Management System! (A CLI based system developed by amlakhani)"
    );
  });
}
  ...
  if (answer.operation === "Exit") {
    ...
    if (!courses.length) {
      ...
      courses.map((course, i) => {
        console.log(
          chalk.white(`${i + 1}: ${course.courseName} (ID: ${course.courseId})`)
        );
      });
      console.log("\n");
      askForOperation();
    ...
    if (!isCourseExist) {
      ...
      if (isAnyStudentEnrolled) {
        spinner.error({
          text: chalk.whiteBright.bgRed(
            "Please unenroll student(s) from this course before deleting!\n"
          ),
        });
      } else {
    ...
    const spinner = createSpinner(
      "getting student's data, please wait..."
    ).start();
    await sleep();

    const isStudentExist = students.findIndex(
      (student) => student.studentId === studentId
    );
    ...
    if (isStudentExist === -1) {
      ...
      const spinner2 = createSpinner(
        "updating student's data, please wait..."
      ).start();
      await sleep();
    ...
    const spinner = createSpinner(
      "getting course data, please wait..."
    ).start();
    await sleep();

    const isCourseExist = courses.findIndex(
      (course) => course.courseId === courseId
    );
    ...
    if (isCourseExist === -1) {
      ...
      const spinner2 = createSpinner(
        "updating course data, please wait..."
      ).start();
      await sleep();
    ...
    const spinner = createSpinner(
      "un-enrolling student, please wait..."
    ).start();
    await sleep();
    ...
    if (isStudentExist === -1) {
      ...
      const cIndex = updatedCourses.findIndex(
        (course) => course === courses[isCourseExist].courseId
      );
    ...
    if (isStudentExist === -1) {
      ...
      console.log(`
        ...
        ${chalk.white(
          `- Enrolled Courses: ${students[isStudentExist].courses
            .map(
              (course) => courses.find((c) => c.courseId === course).courseName
            )
            .join(", ")}`
        )}
      `);
    ...
    if (isStudentExist === -1) {
      ...
      if (students[isStudentExist].balance <= 0) {
        ...
        console.log(`
          ${chalk.white(
            `- Current Balance: ${chalk.green(
              students[isStudentExist].balance
            )}`
          )}
        `);
