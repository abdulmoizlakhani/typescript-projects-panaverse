...
let currentUser = users[Math.floor(Math.random() * 5)];
const sleep = (ms = 2000) => new Promise((r) => setTimeout(r, ms));
function welcome() {
    const msg = `ATM MACHINE`;
    figlet(msg, (err, data) => {
        console.log(gradient.pastel.multiline(data));
        rainbowTitle = chalkAnimation.rainbow("Welcome to ATM Machine! (A CLI based ATM simulation developed by amlakhani)");
    });
}
...
async function askForOperation() {
    ...
    if (answer.operation === "Exit") {
        ...
        console.log(chalk.white(`Your current balance is ${chalk.blueBright.bgCyanBright(`PKR ${currentUser.currentBalance}`)}`));
        askForOperation();
    }
        ...
        if (deposit <= 0) {
            ...
            spinner.success({
                text: chalk.white(`Your new account balance balance is ${chalk.blueBright.bgCyanBright(`PKR ${currentUser.currentBalance}`)}`),
            });
        }
        askForOperation();
    }
