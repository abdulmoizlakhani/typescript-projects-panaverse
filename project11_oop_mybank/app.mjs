...
function welcome() {
    ...
    figlet(msg, (err, data) => {
        ...
        rainbowTitle = chalkAnimation.rainbow("Welcome to Ninja Bank! (A CLI based banking system developed by amlakhani)");
    });
}
async function startProgram() {
    ...
    if (answer.operation === "Exit") {
        ...
        const spinner = createSpinner("creating bank account, please wait...").start();
        await sleep();
        spinner.success({
            text: chalk.whiteBright.bgGreen("Congratulations! You account has been created!\n"),
        });
        startProgram();
    }
        ...
        if (msg === "Invalid Pin Code or Account doesn't exist!") {
            ...
            const spinner = createSpinner("transaction in progress, please wait...").start();
            await sleep();
            spinner.success({
