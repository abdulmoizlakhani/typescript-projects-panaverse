...
function welcome() {
    ...
    figlet(msg, (err, data) => {
        ...
        rainbowTitle = chalkAnimation.rainbow("Welcome to TO-DOO! (A CLI based todo list app developed by amlakhani)");
    });
}
...
async function askForOperation() {
    ...
    if (answer.operation === "Exit") {
        ...
        if (!todoList.length) {
            ...
            todoList.map((todo, i) => {
                console.log(chalk.white(`${i + 1}: ${todo.text} ${todo.done
                    ? `${chalk.green("(Done)")}`
        ...
        const spinner = createSpinner("marking todo as done, please wait...").start();
        await sleep();
        if (todoIndex <= 0) {
            spinner.error({
        ...
        const spinner = createSpinner("marking todo as undone, please wait...").start();
        await sleep();
        if (todoIndex <= 0) {
            spinner.error({
                text: chalk.whiteBright.bgRed("Index can't be 0 or less!\n"),
